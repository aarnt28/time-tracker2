...
def migrate_schema():
    ...
    with engine.begin() as conn:
        cols = {
            row[1] for row in conn.exec_driver_sql("PRAGMA table_info(entries)").all()
        }
        ...
        if "rounded_minutes" not in cols:
            conn.exec_driver_sql(
                "ALTER TABLE entries ADD COLUMN rounded_minutes INTEGER NOT NULL DEFAULT 0"
            )
        if "rounded_hours" not in cols:
            conn.exec_driver_sql(
                "ALTER TABLE entries ADD COLUMN rounded_hours TEXT NOT NULL DEFAULT '0.00'"
            )
        if "elapsed_minutes" not in cols:
            conn.exec_driver_sql(
                "ALTER TABLE entries ADD COLUMN elapsed_minutes INTEGER NOT NULL DEFAULT 0"
            )
            conn.exec_driver_sql(
                "UPDATE entries SET elapsed_minutes = COALESCE(elapsed_minutes, 0)"
            )
        if "invoice_number" not in cols:
            conn.exec_driver_sql("ALTER TABLE entries ADD COLUMN invoice_number TEXT")
            cols2 = {
                row[1]
                for row in conn.exec_driver_sql("PRAGMA table_info(entries)").all()
            }
            if "invoice" in cols2:
                try:
                    conn.exec_driver_sql(
                        "UPDATE entries SET invoice_number = invoice WHERE invoice_number IS NULL"
                    )
                except Exception:
                    pass
...
def _client_table_path() -> Path:
    ...
    for p in [
        BASE_DIR / "client_table.json",
        BASE_DIR / "app" / "client_table.json",
        DATA_DIR / "client_table.json",
    ]:
        if p.exists():
            return p
...
def _fetch_rows(
    ...
    qry = db.query(Entry)
    if client:
        qry = qry.filter(Entry.client == client)
    if client_key:
        qry = qry.filter(Entry.client_key == client_key)
    if status == "open":
        qry = qry.filter(Entry.completed == 0)
    if status == "done":
        qry = qry.filter(Entry.completed == 1)
    if qtext:
        like = f"%{qtext}%"
        qry = qry.filter(
            or_(
                Entry.client.like(like),
...
def ui_set_invoice(
    entry_id: int, invoice_number: str = Form(""), db: Session = Depends(get_db)
):
    r = db.get(Entry, entry_id)
    if not r:
        raise HTTPException(404, "Not found")
...
async def ui_import(csvfile: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    for row in reader:
        ...
        client_key = (
            row.get("client_key")
            or row.get("Client Key")
            or roster_key
            or safe_client_key(client)
        )
        start_iso = (
            row.get("start_iso") or row.get("Start ISO") or now_local().isoformat()
        )
...
def api_sessions_start(
    payload: SessionStart, db: Session = Depends(get_db)
) -> Dict[str, Any]:
...
def api_sessions_stop(
    payload: SessionStop, db: Session = Depends(get_db)
) -> Dict[str, Any]:
    client = payload.client.strip()
    if not client:
        raise HTTPException(400, "client is required")
    ...
    if payload.note and payload.note.strip():
        r.note = (
            (r.note + ("\n" if r.note else "") + payload.note.strip())
            if r.note
